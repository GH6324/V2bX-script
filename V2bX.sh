...
install() {
    bash <(curl -Ls https://gh-proxy.com/https://raw.githubusercontents.com/GH6324/V2bX-script/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://gh-proxy.com/https://raw.githubusercontent.com/wyx2685/V2bX-script/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
install_bbr() {
    bash <(curl -L -s https://gh-proxy.com/https://github.com/ylx2016/Linux-NetSpeed/raw/master/tcpx.sh)
}

update_shell() {
    wget -O /usr/bin/V2bX -N --no-check-certificate https://gh-proxy.com/https://raw.githubusercontent.com/GH6324/V2bX-script/master/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
